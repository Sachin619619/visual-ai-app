...
const QUICK_PROMPTS = [
  ...
  // Showcases: flip cards, radar chart, leaderboard, aurora, shimmer
  { key: 'cognitive-biases', prompt: 'Create a stunning cognitive biases visual guide — show 8 major biases (Confirmation Bias, Dunning-Kruger, Anchoring, Recency Bias, Sunk Cost, Halo Effect, FOMO, Availability Heuristic) as 3D flip cards (front = bias name + icon, back = definition + real-world example). Add a leaderboard of "most costly biases in investing" with impact score bars. Dark psychology theme with violet accents.', label: '🧩 Cognitive Biases' },
  { key: 'city-rankings', prompt: 'Create a global city comparison dashboard — compare Tokyo, NYC, London, Singapore, Dubai, Bangalore across 6 dimensions (Cost of Living, Safety, Culture, Jobs, Weather, Quality of Life) using a multi-series radar/spider chart. Add a ranked leaderboard with overall score, delta vs last year, and category badges. Use aurora-bg hero banner with shimmer-text for the title. Dark premium theme.', label: '🌆 City Rankings' },
  { key: 'programming-languages', prompt: 'Create a programming languages comparison visual — compare Python, JavaScript, Rust, Go, Java, Swift across Speed, Learning Curve, Ecosystem, Job Demand, Community, and Performance using a radar chart with 6 axes. Add a stacked horizontal bar showing language popularity by domain (Web, ML, Systems, Mobile, Backend). Ranked leaderboard by GitHub stars with trend arrows. Dark tech cyan/violet theme.', label: '💻 Language Wars' },
  { key: 'mars-colonization', prompt: 'Create a breathtaking Mars colonization infographic — use aurora-bg hero with shimmer-text headline "Becoming Multi-Planetary". Show journey phases as animated step tracker, key survival challenges (radiation, atmosphere, food, water) as circular gauges, habitat blueprint as SVG diagram, mission cost bar chart, and SpaceX/NASA timeline. Deep space crimson/orange/indigo theme.', label: '🚀 Mars Colonization' },
  { key: 'workout-science', prompt: 'Create a workout science infographic — show muscle fiber types (Type I vs Type II) as a flip card comparison, weekly training periodization as a stacked horizontal bar chart (strength/hypertrophy/endurance/recovery blocks), top exercises ranked by calorie burn as a leaderboard, heart rate zones as a gauge cluster, and recovery time table. Dark athletic theme with lime/amber accents.', label: '🏋️ Workout Science' },
  { key: 'mental-health-stats', prompt: 'Create a mental health awareness dashboard — global depression/anxiety prevalence donut chart, treatment gap bar chart by country, therapy effectiveness comparison (CBT, medication, meditation) as a radar chart, stigma reduction trends line chart, and self-care practices ranked by evidence strength as a leaderboard. Warm empathetic teal/purple theme.', label: '💚 Mental Health' },
  { key: 'history-of-money', prompt: 'Create a stunning history of money infographic — use aurora-bg hero with shimmer-text "From Barter to Bitcoin". Show evolution as a vertical timeline (barter → commodity money → coins → paper → credit → crypto), inflation rate by decade bar chart, global currency adoption pie chart, and Bitcoin vs gold vs USD returns comparison. Finance gold/amber theme.', label: '💰 History of Money' },
  { key: 'social-media-algorithms', prompt: 'Create a social media algorithm explainer — show how the feed ranking works as an animated flow diagram (signal collection → ML scoring → filtering → ranking). Use flip cards for each major platform (Instagram, TikTok, YouTube, LinkedIn) showing their key ranking signals. Add engagement rate comparison radar chart across platforms, and a leaderboard of content types by average reach. Dark neon theme.', label: '📲 Social Algorithms' },
  // Showcases: waterfall chart, split hero, stagger, neon borders, holo-text
  { key: 'fire-calculator', prompt: 'Create a FIRE (Financial Independence, Retire Early) visual guide — use a split hero with "Your Path to FIRE" heading left + compound growth line chart right. Show the waterfall/bridge chart of income → savings → expenses → invested → FIRE number. Add stagger-children grid of FIRE milestones (25x expenses, 4% rule, etc.), progress bars for savings rate targets (10%, 20%, 50%), and a timeline of reaching financial independence at different savings rates. Use number-pop for key stats. Finance gold/emerald theme.', label: '🔥 FIRE Calculator' },
  { key: 'olympics-stats', prompt: 'Create a stunning Olympics statistics dashboard — use a split hero with medal tally and a country bar chart. Show all-time medal leaderboard with holo-text gold/silver/bronze badges and delta indicators. Add stagger-children grid of top sports by medal count, radar chart comparing USA vs China vs UK vs Germany across Summer/Winter/Team/Individual events, and a timeline of host cities 1896-2024. Neon-border-amber on gold medal stats. Bright Olympic rings-inspired palette.', label: '🏅 Olympics Stats' },
  { key: 'spotify-wrapped', prompt: 'Create a Spotify Wrapped-style music year-in-review dashboard — use aurora-bg hero with shimmer-text "Your 2024 Wrapped". Show top 5 artists as stagger-children cards with neon-border-green + holo-text rank badges. Add minutes listened stat with number-pop animation, top genres as a polar area chart, listening pattern heatmap calendar (weekday × hour grid), and a "sound personality" radar chart (energy, danceability, mood, acousticness). Vibrant Spotify dark green/black theme.', label: '🎵 Spotify Wrapped' },
  { key: 'tech-salaries', prompt: 'Create a tech salary visual guide — use a split hero with "Software Engineer Pay in 2025" + horizontal bar chart of top-paying companies. Show waterfall/bridge chart: base salary → equity → bonus → total comp. Add stagger-children cards for salary by role (SWE, PM, Data Scientist, ML Engineer) with progress bars vs industry average, radar chart comparing FAANG vs startups vs big tech across comp/wlb/growth/impact, and leaderboard of top-paying cities with delta. Finance dark blue/gold theme.', label: '💼 Tech Salaries' },
  { key: 'premier-league', prompt: 'Create a Premier League 2024/25 season dashboard — use aurora-bg hero with shimmer-text title. Show league table as a stagger-children leaderboard with progress bars for points, goal difference indicators, and champion/relegated zone color coding. Add top scorers horizontal bar chart with neon-border-violet on golden boot leader, goals per game scatter plot, and a form guide grid (last 5 results as W/D/L colored dots). Animated stat cards for total goals, clean sheets, cards. Dark football stadium theme.', label: '⚽ Premier League' },
  { key: 'stoicism-guide', prompt: 'Create a Stoicism philosophy visual guide — use an elegant split hero with a Marcus Aurelius quote left + core principles radar chart right. Show 4 cardinal virtues (Wisdom, Justice, Courage, Temperance) as 3D flip cards (front = virtue name + icon, back = definition + daily practice). Add a stagger-children grid of 8 Stoic exercises with neon-border-cyan cards, "Dichotomy of Control" visual (inner circle = control, outer ring = not control), and a timeline of great Stoics from Zeno to Marcus Aurelius. Dark philosophy gold/stone theme.', label: '🏛️ Stoicism Guide' },
  { key: 'climate-tipping-points', prompt: 'Create a climate tipping points visual dashboard — use an aurora-bg hero with glitch-text "CRITICAL THRESHOLDS". Show 8 major tipping points (ice sheets, permafrost, Amazon, coral reefs, etc.) as circular gauges showing % toward the threshold, each with a neon-border-red or neon-border-amber glow. Add temperature anomaly line chart, CO2 concentration gauge, a cascade diagram showing how tipping points trigger each other with SVG arrows, and probability bars. Dark urgent red/orange theme.', label: '🌡️ Climate Tipping Points' },
  { key: 'human-evolution', prompt: 'Create a human evolution visual timeline — use a horizontal scrollable timeline from Australopithecus (4M years ago) to Homo Sapiens (300K years ago), each species as a stagger-children card with brain size, height, and key innovation. Add a radar chart comparing species across intelligence, tool use, social complexity, and language. Show brain volume growth as an animated line chart, migration map as an SVG world outline with arrow paths, and skull comparison as a flip card (profile view front, key features back). Deep earth amber/ochre/indigo theme.', label: '🦴 Human Evolution' },
  // CSS-only viz primitive showcases: .donut, .heat-grid, .funnel, .sparkline
  { key: 'hiring-pipeline', prompt: 'Create a recruiting & hiring pipeline dashboard — use the CSS .funnel component to show the full hiring funnel: Applied(2,400) → Screened(680) → Phone Interview(290) → Technical(140) → Final Round(58) → Offer(24) → Hired(18), with conversion % drop between each stage in red. Add a stagger-children grid of recruiter stat cards (time-to-hire, cost-per-hire, offer acceptance rate) with number-pop. Show source-of-hire breakdown as 4 CSS .donut rings (LinkedIn, Referral, Job Board, Direct) side by side. Include a 7-day × 12-week .heat-grid showing interview volume intensity. Dark HR/talent theme with violet/blue accents.', label: '🧑‍💼 Hiring Pipeline' },
  { key: 'energy-mix', prompt: 'Create a global energy mix dashboard — show each major country\'s (USA, China, Germany, Norway, India) renewable energy share as a CSS .donut ring (--pct and --clr per country, green shades for high renewable). Add a CSS .funnel showing energy transformation losses: Generation(100%) → Grid Transmission(92%) → Distribution(85%) → End Consumer(78%). Include a .sparkline for each country\'s renewable growth over the last 10 years inside the stat row. Add a stagger-children grid of energy source cards (Solar, Wind, Hydro, Nuclear, Gas, Coal) with progress bars. Dark green sustainability palette.', label: '⚡ Energy Mix' },
  { key: 'dev-activity', prompt: 'Create a developer productivity year-in-review dashboard — use a .heat-grid (52 columns × 7 rows) showing GitHub-style contribution intensity across the full year, with .heat-cell-green variant and --intensity mapped to commit count. Show top stats (total commits, PRs merged, issues closed, streak) as number-pop badges in a stagger-children row. Add .sparkline bar charts embedded inside each month column showing commit volume per week, a donut ring for language split (TypeScript, Python, Go, etc.), and a radar chart for contribution categories (features, fixes, docs, reviews, infra). Dark code terminal theme green/cyan.', label: '💻 Dev Activity' },
  { key: 'saas-metrics', prompt: 'Create a SaaS business metrics dashboard — show MRR, ARR, NRR, CAC, LTV in a stagger-children stat table where EACH row has a .sparkline trend bar embedded (last 6 months). Add .donut rings for revenue split by plan tier (Starter, Pro, Enterprise) and a CSS .funnel for the growth funnel: Website Visitors → Trial Signups → Activated → Paying → Expanded. Include a 5×7 .heat-grid showing churn events by cohort month vs age. Finance dark blue/gold SaaS theme with number-pop on key metrics.', label: '📈 SaaS Metrics' },
  { key: 'portfolio-allocation', prompt: 'Create an investment portfolio allocation dashboard — show a cluster of 6 CSS .donut rings (one per asset class: Stocks 45%, Bonds 20%, Real Estate 15%, Crypto 8%, Gold 7%, Cash 5%) with color-coded --clr per asset. Add a stagger-children performance table with .sparkline trends for each asset class over 12 months. Show risk-return scatter plot (Chart.js bubble), rebalancing waterfall chart (target vs current allocation), and a portfolio health score gauge. Finance dark blue/amber theme with number-pop on total portfolio value.', label: '💼 Portfolio Tracker' },
  { key: 'content-funnel', prompt: 'Create a content marketing funnel analytics dashboard — use the CSS .funnel to show audience journey: Impressions(500K) → Clicks(32K) → Page Views(18K) → Leads(2,400) → MQLs(480) → SQLs(96) → Customers(28). Show CTR and conversion at each step. Add a 7×24 .heat-grid showing peak content engagement by weekday × hour (heat-cell-cyan). Include .sparkline per content channel (Blog, YouTube, Podcast, Social) showing weekly traffic, top performing posts as a stagger-children leaderboard with engagement score bars. Dark marketing analytics theme violet/orange.', label: '📣 Content Funnel' },
  { key: 'fitness-year', prompt: 'Create a fitness year-in-review dashboard — use a .heat-grid (52×7) for workout activity calendar with heat-cell-green intensity (rest=0, light=0.3, moderate=0.6, intense=1.0). Show total workouts, km run, calories burned as number-pop stat cards. Add .donut rings for workout type split (Running, Strength, Cycling, HIIT, Yoga) with colored --clr. Include .sparkline bar charts for weekly training volume per month, a progress bar for each personal record vs goal (5K time, max bench, deadlift), and heart rate zone distribution donut. Dark athletic theme lime/amber.', label: '🏃 Fitness Year Review' },
  { key: 'social-engagement', prompt: 'Create a social media engagement analytics dashboard — show platform performance as a stagger-children table (Instagram, TikTok, YouTube, Twitter, LinkedIn) with .sparkline trend bars embedded in the engagement column. Add a 7×24 .heat-grid showing best posting times by weekday × hour (heat-cell-cyan). Show follower growth split by platform as CSS .donut rings side by side. Include content type performance funnel: Posted→Served→Seen→Engaged→Shared→Converted using the .funnel component. Top posts leaderboard with holo-text rank badges. Dark social media neon theme.', label: '📲 Social Engagement' },
];
